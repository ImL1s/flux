...
class FluxDevServer {
  ...
  Future<void> start() async {
    ...
    final handler = webSocketHandler((WebSocketChannel channel, String? subprotocol) {
  ...
  void _sendScriptList(WebSocketChannel channel) {
    ...
    final fluxFiles = dir.listSync(recursive: true)
        .whereType<File>()
  ...
  void _recompileAndBroadcast(String path) {
    try {
      ...
      final scriptName = path.split(Platform.pathSeparator).last.replaceAll('.flux', '');
