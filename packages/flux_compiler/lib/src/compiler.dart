...
class Compiler {
  ...
  Compiler._inner(this._enclosing, String name)
      : _moduleName = _enclosing?._moduleName {
  ...
  void _emit(OpCode op, int line, int column) {
    if (_sourceMapGenerator != null) {
      _sourceMapGenerator!.addEntry(
        ...
        sourceColumn: column,
        sourceIndex:
            0, // Assuming single source file for now or handled by generator addSource logic
      );
    }
  ...
  void _compileWidgetDecl(WidgetDecl stmt) {
    ...
    final widgetObj = CompiledWidget(
      ...
      stateInitializers: stmt.stateFields.map((f) {
        ...
        final initCompiler =
            Compiler._inner(this, "${stmt.name}.state.${f.name}");
  ...
  void _compileFunctionDecl(FunctionDecl stmt) {
    ...
    funcCompiler._function.localNames =
        funcCompiler._locals.map((l) => l.name).toList();
  ...
  void _compileForStmt(ForStmt stmt) {
    ...
    if (stmt.condition != null) {
      _compileExpression(stmt.condition!);
      exitJump = _emitJump(
          OpCode.jumpIfFalse, stmt.condition!.line, stmt.condition!.column);
  ...
  void _compileBinary(BinaryExpr expr) {
    ...
    switch (expr.operator_.type) {
      case TokenType.plus:
        _emit(OpCode.add, expr.line, expr.column);
        break;
      case TokenType.minus:
        _emit(OpCode.sub, expr.line, expr.column);
        break;
      case TokenType.star:
        _emit(OpCode.mul, expr.line, expr.column);
        break;
      case TokenType.slash:
        _emit(OpCode.div, expr.line, expr.column);
        break;
      case TokenType.percent:
        _emit(OpCode.mod, expr.line, expr.column);
        break;
      ...
      case TokenType.notEqual:
        ...
        break;
      case TokenType.less:
        _emit(OpCode.less, expr.line, expr.column);
        break;
      case TokenType.greater:
        _emit(OpCode.greater, expr.line, expr.column);
        break;
      case TokenType.lessEqual:
        _emit(OpCode.lessEqual, expr.line, expr.column);
        break;
      case TokenType.greaterEqual:
        _emit(OpCode.greaterEqual, expr.line, expr.column);
        break;
      default:
        break;
    }
  ...
  void _compileLiteral(LiteralExpr expr) {
    if (expr.value == null) {
      ...
      _emit((expr.value as bool) ? OpCode.true_ : OpCode.false_, expr.line,
          expr.column);
    } else {
  ...
  void _compileUnary(UnaryExpr expr) {
    ...
    switch (expr.operator_.type) {
      case TokenType.minus:
        _emit(OpCode.negate, expr.line, expr.column);
        break;
      case TokenType.not:
        _emit(OpCode.not, expr.line, expr.column);
        break;
  ...
  void _compileCall(CallExpr expr) {
    if (expr.callee is VariableExpr &&
        (expr.callee as VariableExpr).name == 'print') {
      if (expr.arguments.length != 1)
        throw Exception("print() takes 1 argument.");
  ...
  void _compileClassDecl(ClassDecl stmt) {
    ...
    // Compile 'init' method (constructor) with field initializers
    {
      ...
      for (final field in stmt.fields) {
        if (field.initializer != null) {
          ...
          methodCompiler._emit(OpCode.pop, field.line,
              field.column); // Clean up stack (setProperty pushes value)
        }
      }
      ...
      methodCompiler._function = CompiledFunction(
        ...
        arity: initMethod?.parameters.length ??
            0, // Arity is based on explicit init method, or 0 for default
      );

      // Add local names for debugging
      methodCompiler._function.localNames =
          methodCompiler._locals.map((l) => l.name).toList();
    ...
    for (final member in otherMethods) {
      final methodCompiler =
          Compiler._inner(this, '${stmt.name}.${member.name}');
      methodCompiler._beginScope(); // Start a scope for 'this' and parameters
      ...
      if (methodCompiler.chunk.code.isEmpty ||
          methodCompiler.chunk.code.last != OpCode.return_) {
      ...
      methodCompiler
          .endCompiler(member.line); // End the compiler for this method
    }
...
/// [variableNames] is the list of local variables available in the scope,
/// ordered by their stack slot index.
CompiledFunction compileFluxExpression(
    String source, List<String> variableNames,
    {bool generateSourceMap = false, String moduleName = '<eval>'}) {
  ...
  final compiler =
      Compiler(moduleName: moduleName, generateSourceMap: generateSourceMap);
