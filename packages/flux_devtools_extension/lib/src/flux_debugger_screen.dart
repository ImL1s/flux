...
class _FluxDebuggerScreenState extends State<FluxDebuggerScreen> {
  ...
  Future<void> _fetchStackAndLocals() async {
    try {
      // Get Stack
      final stackResp = await serviceManager.callServiceExtensionOnMainIsolate('ext.flux.getStack');
  ...
  Future<void> _fetchLocals(int frameIndex) async {
    try {
      final localsResp = await serviceManager.callServiceExtensionOnMainIsolate('ext.flux.getLocals', args: {
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: const Text('Scripts', style: TextStyle(fontWeight: FontWeight.bold)),
                      ),
                      Expanded(
                      ...
                      Expanded(
                        ...
                        child: ConsolePane(
                          ...
                          onEvaluate: (expr) async {
                             final resp = await serviceManager.callServiceExtensionOnMainIsolate('ext.flux.eval', args: {
                               'expr': expr,
    ...
    return Column(
      children: [
        Container(
           ...
           color: Colors.grey.shade200,
           width: double.infinity,
           child: Text(scriptName, style: const TextStyle(fontWeight: FontWeight.bold)),
        ),
        Expanded(
          child: ListView.builder(
