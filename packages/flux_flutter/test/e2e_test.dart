...
void main() {
  ...
  testWidgets('E2E: Todo App (Input, State Update, List Rendering)', (WidgetTester tester) async {
    ...
    await tester.pump(); // Rebuild with input state updated (if optimistic) or just event processed
    
    // 2. Click Add calls push(todos, "Buy Milk") and clears input
    await tester.tap(find.text('Add'));
    await tester.pump(); // Process event
    await tester.pump(const Duration(milliseconds: 100)); // Allow state update to trigger rebuild
