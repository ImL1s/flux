...
Chunk compile(String source) {
  ...
  if (parser.errors.isNotEmpty) {
    throw Exception(
        'Compilation failed with ${parser.errors.length} errors: ${parser.errors}');
  }
...
void main() {
  group('Flux Hot Reload Comprehensive Tests', () {
    testWidgets('Preserves matching state when adding new state fields',
    ...
    testWidgets('Handles removing state fields gracefully',
        (WidgetTester tester) async {
      final v1 = '''
        widget TestWidget {
          state a = "kept";
      ...
      await tester.pumpWidget(MaterialApp(
          home: FluxWidget(widgetName: 'TestWidget', runtime: runtime)));
    ...
    testWidgets('Updates closure logic while preserving captured state',
        (WidgetTester tester) async {
      final v1 = '''
        widget TestWidget {
          state count = 0;
    ...
    testWidgets('Handles syntax errors by throwing (handled by wrapper in app)',
        (WidgetTester tester) async {
