...
void main() {
  ...
  test('http.get returns standardized response map', () async {
    ...
    final mockClient = MockClient((request) async {
       if (request.method == 'GET' && request.url.toString() == 'https://example.com') {
          return http.Response('{"foo": "bar"}', 200, headers: {'content-type': 'application/json'});
       }
  ...
  test('http.post returns response', () async {
    final args = ['https://api.com', {'body': 'data'}];
    final fn = httpModule.get('post') as AsyncNativeFunction;

    final mockClient = MockClient((request) async {
       if (request.method == 'POST' && request.url.toString() == 'https://api.com') {
